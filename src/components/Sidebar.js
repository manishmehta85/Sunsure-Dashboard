...
export default function Sidebar() {
  ...
  useEffect(() => {
    supabase.from('projects').select('id,name,state').order('name').then(({ data }) => setProjects(data || []));
  }, []);
  ...
  const ROLE_COLORS = { admin:'var(--orange)', editor:'var(--blue)', viewer:'var(--text3)' };
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-projects">
        {projects.map(p => (
          <NavLink key={p.id} to={`/project/${p.id}`} className={({isActive})=>isActive?'proj-item active':'proj-item'}>
      ...
      {can.edit && (
        <button className="sidebar-new-project" onClick={()=>navigate('/')}>+ New Project</button>
      )}
